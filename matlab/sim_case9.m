...
%%%%%%%%%%%%%%%%%%%%%%%%%%%
ps = updateps(case9_ps); % load the dataset

ps.branch(:, C.br.tap) = 1; % tap ratio for transformer

ps.shunt(:, C.sh.frac_S) = 1; % fraction of the shunt (P/Q) that is constant complex power
ps.shunt(:, C.sh.frac_E) = 0; % fraction of the shunt that should use an expontential load model P*Vmag^gamma;
ps.shunt(:, C.sh.frac_Z) = 0; % fraction of the shunt (P/Q) that is constant complex impedance
ps.shunt(:, C.sh.gamma) = 0.08; % the exponent gamma in the equation above.

%%%%%%%%%%%%%%%%%%%%%%%%%%%
% to differentiate the line MVA ratings if they are the same % ??? what if they don't ???
%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
opt.nr.use_fsolve = true; % ??? what is fsolve ???
...
% COI doesn't work for islanding scenario for now % ??? what is COI (Center of inertia)???
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%
ps = newpf(ps, opt); % ???
[ps.Ybus, ps.Yf, ps.Yt] = getYbus(ps, false); % Extract the Ybus matrix from power system data
ps = update_load_freq_source(ps); % this function assign a generator freqency signal to each bus.
